...
def building_loader(data_path, building_attributes, weather_file, solar_profile, building_ids,
...
class CityLearn(gym.Env):
    def __init__(self, data_path, building_attributes, weather_file, solar_profile, building_ids,
                 buildings_states_actions=None, simulation_period=(0, 8759),
                 cost_function=['ramping', '1-load_factor', 'average_daily_peak', 'peak_demand',
                                'net_electricity_consumption'], central_agent=False, verbose=0):
        ...
        self.buildings, self.observation_spaces, self.action_spaces, self.observation_space, self.action_space = building_loader(
            data_path, building_attributes, weather_file, solar_profile, building_ids, self.buildings_states_actions)
    ...
    def get_building_information(self):
        ...
        for uid, building in self.buildings.items():
            ...
            for uid_corr, building_corr in self.buildings.items():
                if uid_corr != uid:
                    # print(len(np.array(building.sim_results['dhw_demand'])))
                    # print(np.array(building_corr.sim_results['dhw_demand']))
                    # exit()
    ...
    def cost(self):
        ...
        if self.cost_rbc is None:
            env_rbc = CityLearn(self.data_path, self.building_attributes, self.weather_file, self.solar_profile,
                                self.building_ids, buildings_states_actions=self.buildings_states_actions_filename,
